...
class AOTAutogradCacheTests(CacheKeyEquivalenceMixin, InductorTestCase):
    ...
    @inductor_config.patch("fx_graph_cache", True)
    @functorch_config.patch({"enable_autograd_cache": True})
    def test_region_memory_budget_causes_cache_miss(self):
        """Changing the budget set via torch.autograd.graph.region_memory_budget
        invalidates the AOTAutograd cache."""

        def make_fn(budget):
            def fn(x, y):
                with torch.autograd.graph.region_memory_budget(budget):
    ...
    @inductor_config.patch("fx_graph_cache", True)
    @functorch_config.patch({"enable_autograd_cache": True})
    def test_region_memory_budget_graph_break_cache(self):
        ...
        def make_fn(budget_a, budget_b):
            def fn(x):
                with torch.autograd.graph.region_memory_budget(budget_a):
                    x = (x + 1).relu()
                torch._dynamo.graph_break()
                with torch.autograd.graph.region_memory_budget(budget_b):
                    return (x * 2).relu()
